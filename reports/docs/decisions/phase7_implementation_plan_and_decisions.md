...
**Product:** PULSE | **Phase:** 7 of 7 (final) | **Version:** 1.1.0 (Approved) | **Date:** 2026-08-20  
...
| File                                                                                                                                                        | Status                                                                                                                                                                                                                                                                                    | Notes                                                                                                                                                                                                                                   |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.github/workflows/ci.yml`                                                                                                                                  | **Exists**, but predates downstream test suites. Phase 1 built a baseline `ci.yml` covering lint, type-check, file-size enforcement, and solver-correctness gate — before integration tests, the eval suite, game theory, API streaming, static UI, and post-match report suites existed. | Phase 7 updates the CI pipeline to execute all 176+ tests (including `test_static_ui.py` and `test_match_report_api.py`), enforces the ≥70% coverage gate, runs Trivy container vulnerability scanning, and validates the Docker build. |
| `Dockerfile`                                                                                                                                                | Does not exist. Phase 7 scope.                                                                                                                                                                                                                                                            | Multi-stage Docker build packaging Python runtime, `src/` (including `src/api/static/` presentation layer and `src/analytics/`), and versioned `artifacts/` (models, stratum table, payoff matrices, points dataset).                   |
| `docker-compose.yml`                                                                                                                                        | Does not exist. Phase 7 scope (Finding A).                                                                                                                                                                                                                                                | Exposes port `8000:8000` for the Tactical Cockpit and REST/SSE API, mounts persistent SQLite audit volume at `artifacts/`, enabling `docker compose up --build`.                                                                        |
| Final evaluation report                                                                                                                                     | Does not exist. Phase 7 scope (D-13).                                                                                                                                                                                                                                                     | `reports/docs/evaluations/phase7_final_evaluation_report.md` signed off against `project_charter.md` §5 Definition of Done.                                                                                                             |
| Retrospective escalation-precision evaluation script/report                                                                                                 | Does not exist. Governing methodology: `pulse_ml_canvas.md` §8.                                                                                                                                                                                                                           | `scripts/evaluate_escalation_precision.py` and `reports/docs/evaluations/escalation_precision_report.md`.                                                                                                                               |
| OTel spans in `core/markov_solver.py`, `models/point_win_classifier.py`, `models/pressure_deviation.py`, `core/game_theory.py`, `analytics/match_report.py` | **Partially present** (node-level spans exist in `pulse_graph.py`).                                                                                                                                                                                                                       | Sub-node child spans needed across solver, classifier, deviation estimator, minimax game theory, and whole-match report aggregation.                                                                                                    |
| `src/utils/logger.py`                                                                                                                                       | Exists and is used everywhere via `get_logger(__name__)`.                                                                                                                                                                                                                                 | Uses structured log formatting with RotatingFileHandler and RichHandler console output. Audit pass confirms zero stray `print()` or bare `logging.` calls.                                                                              |
...
**Finding B — two different evaluations are bundled under one task list, and conflating them would be a mistake.** "Run the retrospective escalation-precision evaluation" and "shadow-mode acceptance run" read as adjacent bullets in the same task list, but they're not the same kind of check. The precision evaluation is a **statistical** measurement against `prd.md` §7's headline target (alert precision ≥ 0.75), and its validity depends on being run against data that wasn't used to fit the artifacts being evaluated. The shadow-mode run is an **operational/integration** confirmation — does the full, containerized, end-to-end system run correctly at match scale, streaming live leverage, serving the Tactical Cockpit UI, and generating post-match tactical debriefs. Treating "held-out" as meaning the same thing for both would either weaken the precision evaluation's statistical validity or over-constrain the shadow-mode run for no reason. → **D-1, D-2**.

**Finding C — `llm_client.py` and debrief coverage verified across providers.** Phase 6.6 expanded `src/graph/llm_client.py` with multi-provider async support (Groq Cloud `llama-3.1-8b-instant` and Anthropic `claude-3-5-haiku-20241022`) and deterministic raw fallback, accompanied by mock tests and post-match report test suites (`test_match_report.py`, `test_match_report_api.py`), bringing the full test suite to 176 passing tests. Phase 7 enforces this coverage in CI. → **D-8**.
...
| Option                                                                | Description                                                                                                                                     | Trade-off                                                                                                                                                                                                                                                                                                                                    |
| --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A — Bake artifacts into the image at build time**                   | `COPY artifacts/ ...` during the Docker build; the image is a fully self-contained, versioned unit — one specific artifact state per image tag. | Matches this project's own reproducibility emphasis (bit-identical replay under a fixed seed) extended to the deployable unit itself: the image tag _is_ a specific, traceable artifact version. Larger image, rebuild required on artifact updates — an acceptable cost at this project's current single-node, non-frequent-redeploy scale. |
...
**Context:** per VERIFY item 1, `pulse_ml_canvas.md` §8's actual methodology isn't available in this conversation. This decision can only settle _where_ this evaluation lives, not _how_ it computes precision.
