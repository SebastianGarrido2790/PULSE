...
tracer = trace.get_tracer("pulse.graph.state_monitor")
...
def make_state_monitor_node(
    ...
    async def state_monitor_node(state: PulseGraphState) -> dict[str, Any]:
        ...
        """
        with tracer.start_as_current_span("state_monitor_node") as span:
            ...
            span.set_attribute("pulse.surface", str(ctx.surface))
            span.set_attribute("pulse.serve_number", int(ctx.serve_number))
            ...
            span.set_attribute("pulse.delta_leverage", float(leverage_result.delta_leverage))
            span.set_attribute("pulse.delta_leverage_low", float(lev_low))
            span.set_attribute("pulse.escalate", bool(escalate))
