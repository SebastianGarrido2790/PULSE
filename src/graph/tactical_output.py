...
tracer = trace.get_tracer("pulse.graph.tactical_output")
...
def make_tactical_output_node(
    ...
    async def tactical_output_node(state: PulseGraphState) -> dict[str, Any]:
        ...
        """
        with tracer.start_as_current_span("tactical_output_node") as span:
            ...
            span.set_attribute("pulse.escalated", bool(is_escalated))
            ...
            span.set_attribute("pulse.is_llm_fallback", bool(is_llm_fallback))
            span.set_attribute("pulse.narrative_len", len(narrative))
