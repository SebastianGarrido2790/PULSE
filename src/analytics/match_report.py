...
async def generate_executive_debrief_async(
    ...
    if provider == "groq":
        ...
        if not api_key:
            logger.debug("GROQ_API_KEY not set. Using deterministic executive debrief.")
            return fallback_text
        try:
            import groq
            ...
            response = await client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": POST_MATCH_SYSTEM_PROMPT},
            ...
            logger.warning(
                "Post-match Groq LLM debrief generation failed (%s: %s). Using fallback.",
                type(exc).__name__,
                exc,
            )
        ...
        try:
            ...
            logger.warning(
                "Post-match Anthropic LLM debrief generation failed (%s: %s). Using fallback.",
                type(exc).__name__,
                exc,
            )
